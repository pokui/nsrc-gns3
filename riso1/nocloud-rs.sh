  ...
  for INDEX in $(seq 1 $NGROUPS); do
    ...
    cat <<EOS
      ...
      int set allas;
      {
        ...
        allnet = [ 100.68.${GROUP}.0/24, 100.68.${GROUP}.0/24{32,32},
                   100.68.$(( GROUP+100 )).0/24, 100.68.$(( GROUP+100 )).0/24{32,32},
                   61.45.$(( GROUP+247 )).0/24, 61.45.$(( GROUP+247 )).0/24{32,32},
                   2001:DB8:${GROUP}::/48, 2001:DB8:${GROUP}::/48{128,128},
                   2001:DB8:$(( GROUP+100 ))::/48, 2001:DB8:$(( GROUP+100 ))::/48{128,128}
                 ];
        if ! (net ~ allnet) then reject;
